/**
 * Milestone F4 — Parameter Reflection / Context Breakout Detection Engine
 *
 * Implements high-precision detection of unsanitized input reflection:
 ...
 * 4. Non-destructive canary injection into candidate parameters.
 * 5. Reflection inspection: Raw canary reflected in response body without encoding.
 * 6. Clean abstention with evidence pruning on non-reflecting endpoints (F3).
 * 7. End-to-end evidence-to-candidate pipeline promotion (M49–M54).
 * 8. Canonical Finding generation (type: 'INPUT_VALIDATION_FLAW', severity: 'medium').
 */

import { createHash } from 'node:crypto';
import type {
  ParameterReflectionDetectionRequest,
  ParameterReflectionDetectionResult,
...
function injectParameter(
  ...
  if (endpointUrl.includes(`{${parameterName}}`)) {
    return endpointUrl.replace(`{${parameterName}}`, encodeURIComponent(value));
  }
  if (endpointUrl.includes(`:${parameterName}`)) {
    return endpointUrl.replace(`:${parameterName}`, encodeURIComponent(value));
  }
  const parsed = new URL(endpointUrl);
  parsed.searchParams.set(parameterName, value);
  return parsed.toString();
}

export async function runParameterReflectionDetection(
  request: ParameterReflectionDetectionRequest
): Promise<ParameterReflectionDetectionResult> {
  const lineage = {
  ...
  if (!preflight.ok) {
    return {
      ...
      kind: 'parameter_reflection_detection_result',
  ...
  // 2. Prepare Target URL & Parameters
  let parsedUrl: URL;
  try {
  ...
  // 4. Baseline Probe (Original or Benign Parameter)
  const baselineVal = request.baselineValue ?? 'baseline';
  const baselineUrl = injectParameter(request.endpointUrl, request.parameterName, baselineVal);
  ...
  // 5. Canary Probe (Unique, Safe Non-Destructive Canary)
  const canary = request.canaryPayload ?? `fgc${safeSeed}${Math.random().toString(36).slice(2, 8)}`;
  const reflectionUrl = injectParameter(request.endpointUrl, request.parameterName, canary);

  const validationResponse = await dispatchProbe(
    {
      url: reflectionUrl,
      method,
      headers: { ...sessionHeaders }
  ...
  // Canary is reflected raw in the response body
  const isReflected = validationResponse.bodyText.includes(canary);
  const isBaselineNonReflected = !baselineResponse.bodyText.includes(canary);
  const isSuccessfulResponse = validationResponse.statusCode >= 200 && validationResponse.statusCode < 400;

  if (!isReflected || !isBaselineNonReflected || !isSuccessfulResponse) {
    const abstainedResult: ParameterReflectionDetectionResult = {
      contractVersion: DETECTION_CONTRACT_VERSION,
      kind: 'parameter_reflection_detection_result',
      detectionId: request.detectionId,
      scanId: request.scanId,
      ...
      reasonCode: 'parameter_not_reflected',
      lineage,
      baselineSnapshot,
      validationSnapshot
    };
  ...
  // 7. Response Comparison (M47 - HTTP Difference)
  const comparisonRequest: ResponseComparisonRequest = {
    ...
    thresholds: {
      contentLengthDeltaPercentSignificant: 1,
  ...
  const promotionRequest: HumanReviewedEvidencePromotionRequest = {
    ...
    substancePayload: {
      ...
      difference: {
        ...
        authStateChanged: comparisonResult.difference?.authStateChanged ?? false
      }
    },
    classification: {
  ...
  const finding: Finding = {
    ...
    severity: 'medium',
    title: `Unsanitized Parameter Reflection on ${request.parameterName}`,
    description: `Parameter canary probe proved that input supplied to parameter '${request.parameterName}' is reflected raw in the HTTP response body without context-appropriate encoding.`,
    target: request.endpointUrl,
  ...
  return {
    ...
    reasonCode: 'parameter_reflection_proven',
    lineage,
    reflectedCanary: canary,
    baselineSnapshot,
    validationSnapshot,
...
export class ParameterReflectionDetectionService {
  public async execute(
    request: ParameterReflectionDetectionRequest
  ): Promise<ParameterReflectionDetectionResult> {
    return runParameterReflectionDetection(request);
  }
}
